        ...
        <div className='hero rounded-xl bg-base-200'>
          <div className='hero-content flex-col lg:flex-row '>
            <Image src={products[0].imageUrl} alt={products[0].name}
              className='w-full max-w-sm rounded-lg shadow-2xl' priority width={400} height={800}></Image>
            <div>
              <h1 className='text-5xl font-bold'>{products[0].name}</h1>
              <p className='py-6'>{products[0].description}</p>
              <Link href={'/products/' + products[0].id} className='btn btn-primary'>check this out</Link>
            </div>
          </div>
        </div>
      }
